...
public class Mission4 implements Story {
	...
	public void start_(Character mc) {
		//JOSH START HERE
				...
				if(accepted == true) {
					s.out("A few minutes later "+mc.name_+" arrived at the ramen shop. "+mc.name_+" opened to door to the shop and was greeted by the pleasant smell");
					s.out("of beef flavored ramen. "+mc.name_+" noticed 3 ninja, one was quite tall with short dark hair with leaf village ninja attire, another was");
					...
					answer = s.getIntBetween(1,2);
					if(answer == 1) {
						s.out(mc.name_+" sat next to Thaku.");
						s.out(mc.name_+": \"Are you Thaku?\"");
						s.out("Thaku: \"You're the ninja the leaf village sent me?\" Thaku said incredulously.");
						s.out(mc.name_+": Yes, I'm more powerful than I appear.");
						s.out("Thaku: \"Well, let's hope you're right.\"");
						
						
						
					}else if(answer == 2) {
						s.out(mc.name_+" started walking up to the store clerk and passed the sand ninja. The ninja quickly stood up and blocked "+mc.name_+"'s path.");
